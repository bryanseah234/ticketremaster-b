...
def create_app(test_config=None):
    ...
    if "SQLALCHEMY_DATABASE_URI" not in app.config:
        database_url = os.getenv("TRANSFER_SERVICE_DATABASE_URL") or os.getenv(
            "DATABASE_URL"
        )
        if not database_url:
            raise RuntimeError(
                "TRANSFER_SERVICE_DATABASE_URL (or DATABASE_URL) must be set for transfer-service."
            )
    ...
    from routes import bp as transfers_bp

    app.register_blueprint(transfers_bp)
    _register_error_handlers(app)

    Swagger(app, template={
        "info": {"title": "Transfer Service", "version": "1.0.0"},
        "tags": [{"name": "Health"}, {"name": "Transfers"}],
    })
...
app = None
if os.getenv("TRANSFER_SERVICE_DATABASE_URL") or os.getenv("DATABASE_URL"):
    app = create_app()
