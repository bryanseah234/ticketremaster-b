...
def create_app(test_config=None):
    ...
    if "SQLALCHEMY_DATABASE_URI" not in app.config:
        database_url = os.getenv("TICKET_LOG_SERVICE_DATABASE_URL") or os.getenv(
            "DATABASE_URL"
        )
        if not database_url:
            raise RuntimeError(
                "TICKET_LOG_SERVICE_DATABASE_URL (or DATABASE_URL) must be set for ticket-log-service."
            )
    ...
    from models import TicketLog  # noqa: F401
    from routes import bp as ticket_logs_bp

    app.register_blueprint(ticket_logs_bp)
    ...
    Swagger(app, template={
        "info": {"title": "Ticket Log Service", "version": "1.0.0"},
        "tags": [{"name": "Health"}, {"name": "Ticket Logs"}],
    })
...
app = None
if os.getenv("TICKET_LOG_SERVICE_DATABASE_URL") or os.getenv("DATABASE_URL"):
    app = create_app()
