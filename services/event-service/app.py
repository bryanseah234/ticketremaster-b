...
def create_app(test_config=None):
    ...
    if "SQLALCHEMY_DATABASE_URI" not in app.config:
        database_url = os.getenv("EVENT_SERVICE_DATABASE_URL") or os.getenv(
            "DATABASE_URL"
        )
        if not database_url:
            raise RuntimeError(
                "EVENT_SERVICE_DATABASE_URL (or DATABASE_URL) must be set for event-service."
            )
    ...
    from routes import bp as events_bp
    ...
    Swagger(app, template={
        "info": {"title": "Event Service", "version": "1.0.0"},
        "tags": [
            {"name": "Health"},
            {"name": "Events"},
            {"name": "Admin Events"},
...
app = None
if os.getenv("EVENT_SERVICE_DATABASE_URL") or os.getenv("DATABASE_URL"):
    app = create_app()
